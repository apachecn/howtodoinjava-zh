...
`@Immutable`注解将一个实体标记为不可变。 这在您的实体表示参考数据的情况下非常有用，例如状态列表，性别或其他很少突变的数据。
...
`@Immutable`注解告诉 Hibernate，对不可变实体的任何更新都不应传递给数据库而不会给出任何错误。 `@Immutable`也可以放在集合上； 在这种情况下，对集合的更改（添加或删除）将引发`HibernateException`。
...
在过去的教程中，我们了解了很多有关`@Id`和`@GeneratedValue`注解以为数据库中的记录创建主键的知识。 在大多数实际应用中，这些主键是“***人工主键***”，并且仅在应用运行时内部引用。 但是，还存在“**自然 ID**”的概念，除了人工或复合主键之外，它还提供了另一种方便且合乎逻辑的方式来引用实体。
