...
上面的模块代码看起来不错并达到目的。 但是，在客户端应用程序中时，开发人员想要增加乘法功能 – 除了更改方法`calculate()`中的`SimpleCalculator`类代码外，他别无选择。 此代码不符合 OCP。

#### 3.2。 符合 OCP 的代码

请记住，抽象功能是应用程序中的变化。 在此计算器程序中，calculate 方法中的代码将随每个传入的新操作支持请求而变化。 因此，我们需要在此方法中添加抽象。
...
建议的方法是确定需求中可能更改的部分或难以实施的部分，并将这些部分排除在扩展点之后。
