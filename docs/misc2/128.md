...
这可能是为 DAO 层创建单元测试的第一步。 理想情况下，测试应使用与应用程序相同的配置。 但是可能会有一些更改，这些更改仅针对单元测试。 要解决此问题，您应该**创建另一个测试特定的配置文件，并添加/覆盖测试特定的配置更改**。

例如。 在主应用程序中，如果配置文件为`application-context.xml`，则应创建另一个文件`application-context-test.xml`，然后将原始配置导入到该文件的顶部。 然后覆盖您可能需要的 Bean 定义（例如，使用内存数据库而不是常规数据库）。
