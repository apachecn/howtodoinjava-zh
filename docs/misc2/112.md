...
如果您一直在处理管理大量产品的应用程序，那么您可以轻松地解决此问题：
...
显然，您**希望对象的层次结构可以修改其行为，而不需要修改其源代码**。 怎么做？ 为了解决这个问题，访客模式成为现实。
...
根据 Wikipedia 所述， [**访问者设计模式**](https://en.wikipedia.org/wiki/Visitor_pattern "visitor pattern") 是一种将算法与操作对象的结构分离的方法。 这种分离的实际结果是能够在不修改那些对象结构的情况下向现有对象结构添加新操作。 这是遵循**打开/关闭原则**（ [**SOLID 设计原则**](//howtodoinjava.com/best-practices/5-class-design-principles-solid-in-java/ "5 class design principles [S.O.L.I.D.] in java") 之一）的一种方法。
...
一个例子总是比冗长的理论更好。 因此，让我们在这里也有一个访客设计模式。

假设我们有一个在不同环境中管理路由器的应用程序。 路由器应该能够从其他节点发送和接收字符数据，应用程序应该能够在不同的环境中配置路由器。

本质上，设计应该足够灵活，以支持**路由器将来可以在其他环境中进行配置的方式，而无需对源代码**进行大量修改。
...
要下载上述应用程序的源代码，请遵循给定的链接。

[**Souececode download**](https://docs.google.com/file/d/0B7yo2HclmjI4YlJ2RkYxU3hNWDQ/edit?usp=sharing "visitor design pattern source code")
