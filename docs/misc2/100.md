...
首先，我们需要在问题说明中指定的每个**位置**中都有汽车制造厂。 即 *USACarFactory* ， *AsiaCarFactory* 和 *DefaultCarFactory* 。 现在，我们的应用应该足够智能，可以识别使用的位置，因此我们应该能够使用适当的汽车工厂，甚至不知道内部将使用哪个汽车工厂实现。 这也可以避免我们为某个特定位置称呼错误的工厂。
