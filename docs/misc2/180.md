...
**i）**您有一个用 Struts 编写的旧应用程序，并且想要使用 spring 来提高应用程序的功能很多倍。
**ii）**您确实想根据自己的原因来学习它。
...
Spring 上下文文件 **beans.xml** 是典型的 Spring 单独上下文文件，具有 Web 应用程序运行所需的所有内容，其中包括 struts 正在寻找的 bean 定义`editEmployeeAction`。
...
现在我们必须将休眠集成到应用程序中。 最好的地方是利用 Spring 的强大功能进行集成，以充分利用依赖注入来与不同的 ORM 一起使用。
...
除了 struts + spring + hibernate，我们还使用以下组件来构建应用程序。
 ...
 **c）**在运行此应用程序之前，请不要忘记**设置数据库**。 如果安装不正确，将导致您出现一些异常。
...
如果您打算自己构建应用程序，则在下面给定的**直接在此应用程序中使用的结构**将为您提供帮助。
