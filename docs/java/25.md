...
一切正常，直到您没有在类中覆盖这些方法中的任何一个。 但是，有时应用程序需要更改某些对象的默认行为。 让我们理解**为什么我们需要重写`equals`和`hashCode`方法**。
...
如果您要处理 ORM，请确保**始终使用 getter，并且切勿在`hashCode()`和`equals()` 中使用字段引用**。 这是有原因的，在 ORM 中，字段有时是延迟加载的，直到调用它们的 getter 方法才可用。
