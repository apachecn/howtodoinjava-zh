...
`WatchService`是 JDK 的内部服务，监视注册对象的更改。 这些注册的对象必定是[可监视](https://docs.oracle.com/javase/7/docs/api/java/nio/file/Watchable.html "Watchable interface")接口的实例。 在`WatchService`中注册可监视实例时，我们需要指定我们感兴趣的变更事件的类型。
...
4.  [溢出](https://docs.oracle.com/javase/7/docs/api/java/nio/file/StandardWatchEventKinds.html#OVERFLOW "OVERFLOW")。

您可以在提供的链接中了解这些事件。

`WatchService`接口扩展了[可关闭的](https://docs.oracle.com/javase/7/docs/api/java/io/Closeable.html "Closable interface")接口，表示可以根据需要关闭服务。 通常，应该使用 JVM 提供的关闭挂钩来完成。
...
上述侦听器的构造函数仅采用一个参数，即受监视的配置文件的标准路径。 在文件系统中更改配置文件时，会立即通知`Listener`类。

然后，此侦听器类调用`ApplicationConfiguration.getInstance().initilize(file);`以重新加载到内存缓存中。
...
2.  WatchService provides two methods [take()](https://docs.oracle.com/javase/7/docs/api/java/nio/file/WatchService.html#take%28%29 "Watch Service take method") and [poll()](https://docs.oracle.com/javase/7/docs/api/java/nio/file/WatchService.html#take%28%29 "watch Service poll method"). While `take()` method wait for next change to happen and until it is blocked, `poll()` immediately check for change event.
