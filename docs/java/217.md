...
有效 Java 中的 ***Joshua bloch*** 说：“可以说，所有错误的方法调用都可以归结为非法论点或非法状态，但是其他异常通常用于某些类型的非法论点和状态。 。 如果调用者在某个参数中传递了 null，该参数禁止使用 null 值，则约定将抛出`NullPointerException`而不是`IllegalArgumentException`。”
