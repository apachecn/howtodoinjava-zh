...
在 Java 程序中有效使用并行内核一直是一个挑战。 很少有本地框架可以将工作分配到多个核心，然后将它们加入以返回结果集。 Java 7 已将此功能作为 [**Fork 和 Join 框架**](https://docs.oracle.com/javase/tutorial/essential/concurrency/forkjoin.html "forkjoin")合并。
...
设计好的多线程算法很困难，并且**Fork/Join并非在每种情况下都有效**。 它在其自身的适用范围内非常有用，但是最后，您必须确定您的问题是否适合该框架，否则，**您必须准备在`java.util.concurrent`软件包提供的一流工具基础上开发自己的解决方案**。
