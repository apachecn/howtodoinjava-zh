...
`@Column`注解用于指定字段或属性将映射到的列的详细信息。 其中一些细节与架构相关，因此仅在从注释文件生成架构时才适用。 其他应用则由 Hibernate（或 JPA 2 持久性引擎）在运行时应用并强制执行。 它是可选的，具有一组适当的默认行为，但是在覆盖默认行为或需要将对象模型适合到预先存在的架构中时通常很有用。
