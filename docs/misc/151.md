...
此 [**Hibernate**](//howtodoinjava.com/hibernate-tutorials/ "Hibernate Tutorials") （或 JPA 2）持久化注释教程包含对 Java POJO 进行注释以使其充当持久化 JPA 实体时可能需要的所有重要注释的概述。 本教程首先定义一个 POJO“ *`EmployeeEntity`* ”，在其中定义一些属性，并具有各自的 getter 和 setter 方法。 在学习新注释时，我们将这些注释应用于 EmployeeEntity，然后我们将了解该特定注释的含义。
