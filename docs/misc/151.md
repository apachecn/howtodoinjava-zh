...
在此，已声明了名为 seq1 的序列生成注释。 这指的是称为 HIB_SEQ 的数据库序列对象。 然后，将名称 seq1 引用为`@GeneratedValue`注解的生成器属性。 仅序列生成器名称是必需的； 其他属性将采用合理的默认值，但是作为一种好的做法，您应该为 sequenceName 属性提供一个明确的值。 如果未指定，则持久性供应器将选择要使用的 sequenceName 值。
...
与序列生成器一样，`@TableGenerator`的名称属性是必需的，其他属性是可选的，并且表详细信息由持久性供应器选择。 让我们再来看一个例子。
