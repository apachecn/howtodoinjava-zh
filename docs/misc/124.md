...
Hibernate 是一个开源 **Java 持久性**框架项目。 它使用 HQL 和 SQL 执行功能强大的对象关系映射和查询数据库。 Hibernate 是 Java 中 **ORM 映射**的出色工具。 它可以减少很多复杂性，从而减少应用中的缺陷，否则它们可能会找到存在的方法。 对于那些对 SQL 知识了解有限的开发人员来说，这特别有利。
...
1.  **配置**：通常以`hibernate.properties`或`hibernate.cfg.xml`文件编写。 对于 Java 配置，您可能会发现带有`@Configuration`注释的类。 Session Factory 使用它与 Java 应用和数据库一起使用。 它代表了应用 Java 类型到 SQL 数据库的完整映射集。
