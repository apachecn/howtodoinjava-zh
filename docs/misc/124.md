...
Hibernate 4 带来了许多改进，而`@NaturalId`就是这样很好的改进之一。 如您所知，`@Id`注释用作指定实体主键的元数据。 但是有时，实体通常在 DAO 层代码中使用 ID，该 ID 不是主键，而是其逻辑或自然 ID。 在这种情况下，`@NaturalId`注释将证明是 Hiberate 下命名查询的良好替代。
