...
> **阅读更多： [JPA 2 持久化注释教程](//howtodoinjava.com/hibernate/hibernate-jpa-2-persistence-annotations-tutorial/ "Hibernate / JPA 2 Persistence Annotations Tutorial")**
...
`@Immutable`注释告诉 Hibernate，对不可变实体的任何更新都不应传递给数据库而不会给出任何错误。 `@Immutable`也可以放在集合上； 在这种情况下，对集合的更改（添加或删除）将引发`HibernateException`。
