...
POM 文件在它们之间保持父子关系。 您为项目创建的任何 pom 文件最终都将扩展此 super pom 文件。 从 maven 2.2 开始，这是**超级 pom 文件内容**。
...
您需要在此最小 pom 的基础上添加项目特定的配置。 对于其余配置，如果未指定，则 maven 将使用 super pom 文件中的默认设置。
...
项目使用的 pom 文件是通过合并项目 pom 文件，父 pom 文件（如果有）和 super pom 文件而获得的。 此 pom 文件称为**有效 pom** 文件。
