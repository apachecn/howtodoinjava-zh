...
了解如何使用 **RESTEasy ContainerRequestFilter** 创建安全筛选器，该筛选器能够对基于 RESTEasy 的 Web 应用程序执行认证和授权。
...
*   检查 RolesAllowed 注解，然后从注解中获取所需的角色。 从请求中获取授权信息，并根据应用程序逻辑进行匹配。 如果授权成功，则授予访问权限，否则返回拒绝访问权限。
...
– **更新 方法**
上的@Produces 和@Consumes 注解–启动 tomcat 服务器并测试应用程序。 您将获得理想的结果。
