...
[**PubSub（发布者订阅者）**](https://en.wikipedia.org/wiki/Publish%E2%80%93subscribe_pattern "pubsub") 模型是设计范式，其中多个订阅者正在侦听源上的更改事件，并且一旦发生任何更改，便会立即通知监听器。 在用户交互影响屏幕上多个部分的大型系统中，此模式消除了许多硬编码，并提供了设计灵活性。
