...
缓存是 ORM 框架提供的功能，可帮助用户获得快速运行的 Web 应用程序，同时帮助框架本身减少在单个事务中对数据库的查询数量。 [**Hiberate**](//howtodoinjava.com/hibernate-tutorials/ "hibernate tutorials") 还在两层中提供了此缓存功能。
...
请注意，在 ehcache.xml 中，如果 eternal =“ true”，则为**，那么我们不应编写 timeToIdealSeconds，timeToLiveSeconds** ，因此 hibernate 会注意这些值
...
**timeToLiveSeconds =” seconds”** 表示是否使用此对象的另一个 Session 或类，我的意思是它是否正在被其他会话使用，一旦竞争了时间，情况如何 指定的 timeToLiveSeconds，然后它将被 Hiberate 从全局缓存中删除。
