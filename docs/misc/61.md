...
认证功能在 dropwizard 应用程序中作为单独的模块添加。
...
`Authenticator`类负责验证基本认证标头中包含的用户名/密码凭证。 在企业应用程序中，您可以从数据库中获取用户密码，如果密码匹配，则将用户角色设置为主体对象。 在 dropwizard 中，您将需要实现`io.dropwizard.auth.Authenticator`接口以放置您的应用程序逻辑。
