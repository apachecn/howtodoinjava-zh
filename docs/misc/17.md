...
让我们创建一个 maven 项目，其中包含包**耳**， **war** 和 **jar** 类型的模块。 我们正在创建企业应用程序的结构，其中应用程序将作为 EAR（企业应用程序获取）文件部署到应用程序服务器（例如 weblogic，websphere）。
...
#### 1.2。 创建 Maven 模块-包类型为“耳”

在父项目中创建一个新的 Maven 模块。 将其包类型更改为`'ear'`。 该模块可以具有 maven 耳插件，该插件最终将构建要在服务器上部署的耳文件。
...
与 ear 模块类似，为 war 文件和 service jar 文件再创建两个模块。 分别更改其包并添加 maven 插件。
