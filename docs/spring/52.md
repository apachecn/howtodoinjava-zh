...
实施[`javax.sql.DataSource`](https://docs.oracle.com/javase/10/docs/api/javax/sql/DataSource.html)接口的对象通常会在 JNDI 服务中注册，并且可以使用其 JNDI 名称进行发现。
...
推荐的创建`DataSource` bean 的方法是在带有`@Configuration`注释的类中使用`DataSourceBuilder`类。 数据源也使用基础连接池。
...
在自动装配数据源时，Spring Boot 将首选主数据源，即“ mySqlDataSource” 。 要自动连接另一个非主要数据源，请使用 **@Qualifier** 批注。
...
我们可以配置多个数据源，并且其中之一必须标记为`@Primary`。 默认情况下，主数据源是自动连接的，其他数据源需要与`@Qualifier`注释一起自动连接。
