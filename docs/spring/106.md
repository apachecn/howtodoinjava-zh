...
从[ spring boot 初始化器页面](https://start.spring.io/)开始，这是创建任何基于 spring boot 的应用程序的一个很好的起点。 在这里，我们将仅选择 Config 服务器启动器 pom。 屏幕截图是这样的。 使用此配置，一旦我们生成了项目，便会下载一个 zip 文件，我们将其解压缩后直接导入 eclipse 中。
