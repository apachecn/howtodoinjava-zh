...
学习创建和配置 Spring 批处理的`JobExecutionListener`（作业前后），`StepExecutionListener`（步骤前后），`ItemReadListener`，`ItemProcessListener`，`ItemWriteListener`和`SkipListener`实现。

## JobExecutionListener

#### JobExecutionListener 侦听器示例
