...
现在，在`src`文件夹中的 Spring 运行应用程序类上添加`@org.springframework.cloud.client.discovery.EnableDiscoveryClient`注解。 有了此注解，此工件将像 Spring Discovery 客户端一样发挥作用，并在连接到此服务的领事服务器中注册自己。
