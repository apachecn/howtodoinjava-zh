...
*   `spring-boot-starter-data-jpa`（必填）：它包含 spring 数据，hibernate，HikariCP， [JPA API](https://howtodoinjava.com/jpa-tutorials-and-examples/) ， **JPA 实现（默认 处于 Hibernate 状态）**，JDBC 和其他必需的库。
...
要使用 Spring Boot 测试 Hibernate 配置，我们需要在类中自动连接`EmployeeRepository`依赖项，并使用其方法保存或获取员工实体。
...
将我的问题留在与**和 Spring Boot** 配置 Hibernate 有关的注释部分。

## 6\. Spring Boot Hibernate 教程

1.  [带有 Hibernate](https://howtodoinjava.com/spring-boot2/spring-boot-crud-hibernate/)的 Spring boot crud 操作示例
