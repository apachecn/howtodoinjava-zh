...
在方法级别安全性的先前示例中，我们使用`@PreAuthorize`批注添加了安全性。 注释也是在任何方法上增加安全性的好方法和快速方法。 但是有一个问题，它将您的应用程序代码紧紧地耦合到弹簧上。 至少在理论上就[**最佳做法**](//howtodoinjava.com/java-best-practices/)而言，这都是不推荐的。 推荐的方法是将所有此类安全性定义添加到 xml 配置文件中。 这样，您始终可以更改实现而无需触碰源代码。
