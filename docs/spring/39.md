...
该注解启用 Spring Application Context 的自动配置，并根据类路径中预定义类的存在来尝试猜测和配置我们可能需要的 bean。
...
自动配置类是常规的 Spring Configuration bean。 它们使用`SpringFactoriesLoader`机制定位（针对此类）。 通常，自动配置 bean 是`@Conditional` bean（最常使用`@ConditionalOnClass`和`@ConditionalOnMissingBean`注解）。
