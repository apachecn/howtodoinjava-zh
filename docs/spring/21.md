...
在 Spring 框架中，遵循配置文件中的 bean 依赖关系是一种很好的做法，因此 Spring 容器能够自动关联[协作] Bean 之间的关系。 这意味着可以通过检查 [BeanFactory](https://howtodoinjava.com/spring-core/different-spring-ioc-containers/) 的内容来自动让 Spring 为您的 bean 解决协作者（其他 bean）。 这称为 SpringBean 自动装配。
