...
专门用于注解控制器（例如 Web 控制器）的`@Component`。 它与基于`RequestMapping`注解的处理程序方法结合使用。
...
将构造函数，字段，setter 方法或 config 方法标记为通过依赖项注入自动装配。 我们可以使用`required`属性标记注解的依赖项是否是必需的（强制填充）。 默认情况下，其值为`true`。
...
指示当多个候选者有资格自动装配单值依赖项时，应优先考虑 Bean。当不使用 @Primary 时，我们可能需要提供 @Qualifier 注解以正确注入 bean。在给定的示例中，当自动连接 FooRepository 时，将注入 HibernateFooRepository 的实例 – 直到使用 @Qualifier 注解。
