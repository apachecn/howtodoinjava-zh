...
在软件工程中， [**控件反转**](https://en.wikipedia.org/wiki/Inversion_of_control "inversion_of_control") （IoC）是一种编程技术，其中对象耦合在运行时由汇编程序对象绑定，并且在编译时使用静态分析通常是未知的 。 在这个春季教程中，通过示例了解 ioc 和依赖注入在春季中的区别。
...
在本教程中，我们了解了春季中 ioc 和 di 之间的**区别。**
