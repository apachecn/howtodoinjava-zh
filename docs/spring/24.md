...
Spring 使用`@Scheduled`批注为基于 [cron 表达式](https://en.wikipedia.org/wiki/Cron "cron expression")的任务调度和异步方法执行提供了出色的支持。 可以将@Scheduled 批注与触发器元数据一起添加到方法中。
...
## 1\. Spring @Scheduled 批注

`@Scheduled`批注用于任务调度。 触发信息需要与此注释一起提供。
...
#### 1.2. 如何启用@Scheduled 批注

要在 spring 应用程序中使用`@Scheduled`，必须首先在`applicationConfig.xml`文件中定义 xml 命名空间和模式位置定义。 还添加`task:annotation-driven`以启用基于注释的任务计划。
...
1.  反过来，使用`@Scheduled`注释将使 Spring 容器了解该注释下面的方法将作为作业运行。
2.  请记住，用`@Scheduled`注释的方法不应将参数传递给它们。
3.  他们也不应返回任何值。
4.  如果希望在@Scheduled 方法中使用外部对象，则应使用自动装配将它们注入`DemoService`类，而不是将它们作为参数传递给`@Scheduled`方法。

## 2\. @以固定延迟或固定速率计划

在此方法中，`fixedDelay`属性与`@Scheduled`批注一起使用。 或者，也可以使用`fixedRate`。
...
在此方法中，`cron`属性与`@Scheduled`注释一起使用。 此属性的值必须是 cron 表达式。
...
在此方法中，`cron`属性与`@Scheduled`注释一起使用。 此属性的值必须为 cron 表达式，如先前方法 BUT 一样，该 cron 表达式将在属性文件中定义，并且相关属性的键将用于`@Scheduled`批注。
...
在这种方法中，在属性文件中配置了 cron 表达式，并使用属性密钥为 cron 表达式在配置文件中配置了作业调度。 主要变化是，您无需在任何方法上使用`@Scheduled`批注。 方法配置也在应用程序配置文件中完成。
...
[Download Sourcecode](https://docs.google.com/file/d/0B7yo2HclmjI4Qk5OdkhTeWhxMWc/edit?usp=sharing "Download spring scheduled annotation example")
...
[http://forum.springsource.org/showthread.php?83053-Feature-Scheduled-with-Value-cron-expression](http://forum.springsource.org/showthread.php?83053-Feature-Scheduled-with-Value-cron-expression "Feature-Scheduled-with-Value-cron-expression")