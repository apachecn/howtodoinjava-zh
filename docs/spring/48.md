...
学习在 Spring Boot 应用程序中编写单元测试和集成测试。 了解单元测试和集成测试之间的区别，以及支持此类测试的注释。
...
默认情况下，编写的测试在 Junit 4 中进行。要运行此类测试，我们可以在类级别使用`@RunWith`注释使用[`SpringRunner`](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/test/context/junit4/SpringRunner.html)类（扩展`SpringJUnit4ClassRunner`）。
...
它测试使用`@Mock`对象，首选使用 [MockitoJUnitRunner](https://static.javadoc.io/org.mockito/mockito-core/2.6.8/org/mockito/junit/MockitoJUnitRunner.html) 对象。 它初始化带有 Mock 注释的模拟，因此不需要显式使用`MockitoAnnotations.initMocks(Object)`。 在每种测试方法之前都要初始化模拟。
...
Spring Boot 提供了各种注释，以启用仅与应用程序的某些部分相关的测试基础结构。 它还提供了注释，这些注释也有助于集成测试。 让我们拜访他们。

#### 4.1. @SpringBootTest

该注释有助于编写**集成测试**。 它启动嵌入式服务器并完全初始化应用程序上下文。 我们可以使用`@Autowired`批注将依赖项注入测试类中。
...
此批注用于 [Spring MVC](https://howtodoinjava.com/spring-mvc-tutorial/) 测试。 它禁用完整的[自动配置](https://howtodoinjava.com/spring-boot/springbootapplication-auto-configuration/)，而仅应用与 MVC 测试相关的配置。

它还会自动配置`MockMvc`实例。 通过将`.class`作为注释属性，我们只能初始化一个 Web 控制器。
...
该注释禁用了完全自动配置，而是仅应用与 [WebFlux](https://howtodoinjava.com/spring-webflux/spring-webflux-tutorial/) 测试相关的配置。 默认情况下，用`@WebFluxTest`注释的测试也将自动配置`WebTestClient`。
...
*   `@JdbcTest` – can be used for a typical [jdbc](https://howtodoinjava.com/jdbc-tutorials/) test when a test focuses only on jdbc-based components. It disables full auto-configuration and instead apply only configuration relevant to jdbc tests.
    ...
    默认情况下，用`@JdbcTest`注释的测试是事务性的，并在每个测试结束时回滚。 注释配置内存中嵌入式数据库和`JdbcTemplate`。

*   `@JooqTest` – 当测试仅关注基于 jOOQ 的组件时，可以使用它。 注意，默认情况下，带有@JooqTest 注释的测试使用应用程序配置的数据库。 要使用嵌入式内存数据库，可以使用 `@AutoConfigureTestDatabase`注释来覆盖这些设置。
...
在 SpringBoot 中，任何在顶级类中配置并带有`@TestConfiguration`注释的 Bean 将不会通过组件扫描获得。 我们必须使用包含测试用例的类显式注册`@TestConfiguration`类。

最好的是，这些测试配置不会自动成为应用程序主要配置的一部分。 仅可通过以下两种方式之一按需使用它们以包括此附加测试配置，即

#### 5.1. @导入注释

它称为将一个或多个配置类导入应用程序上下文或弹簧测试上下文中。
...
我们可以在测试类内部的嵌套类中定义测试配置。 嵌套类可以使用 @Configuration 或 @TestConfiguration 注释进行注释。
...
`@Mock`用于模拟创建。 它使测试类更具可读性。 在测试类中，要处理模拟注释，必须至少使用一次`MockitoAnnotations.initMocks(testClass)`。

请注意，如果您正在使用`RunWith(MockitoJUnitRunner.class)`，则无需显式使用`MockitoAnnotations.initMocks()`。 在每种测试方法之前都要初始化模拟。

在不需要弹簧文本上下文的单元测试中使用`@Mock`。

#### 6.2. 没有 Mockito – @MockBean

`@MockBean`批注用于将模拟添加到 Spring ApplicationContext。 它允许模拟类或接口，并记录和验证其行为。

有趣的是，上下文中定义的任何相同类型的现有 bean 都将被该模拟代替。 如果没有定义现有的 bean，将添加一个新的 bean。

`@MockBean`与 Mockito 的`@Mock`类似，但具有 Spring 支持。 我们通常将`@MockBean`与`@WebMvcTest`或`@WebFluxTest`批注一起使用。 这些注释适用于 Web 测试片，并且仅限于单个控制器。
...
Spring Boot 为应用程序及其各种模块的[单元测试](https://howtodoinjava.com/spring-boot2/rest-controller-unit-test-example/)和[集成测试](https://howtodoinjava.com/spring-boot2/spring-integration-testing/)提供了出色的支持。 我们将非常小心地通过使用注释来使用所提供的支持。

使用`@SpringBootTest`批注进行集成测试，而其他自动配置批注则用于特定组件的单元测试。

模拟特定行为是非常普遍的要求，为此，我们可以使用嘲笑者的 @Mock 或 Spring 的 @MockBean 批注。
