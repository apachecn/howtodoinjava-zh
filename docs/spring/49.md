...
鉴于`RestTemplate`类是为调用 REST 服务设计的，因此它的主要方法与 REST 的基础紧密结合在一起就不足为奇了，REST 的基础是 HTTP 协议的方法： **HEAD** ]， **GET** ， **POST** ， **PUT** ， **DELETE** 和**选项**。 例如，`RestTemplate`类具有方法`headForHeaders()`，`getForObject()`，`postForObject()`，`put()`和`delete()`等。
