...
Spring 的关键组件之一是[面向方面编程（AOP）](https://docs.spring.io/spring/docs/current/spring-framework-reference/html/aop.html)框架。 尽管 Spring IoC 容器不依赖于 AOP，这意味着您不需要时就不需要使用 AOP，但 AOP 是对 Spring IoC 的补充，可以提供功能强大的中间件解决方案。 就像 OOP 中模块化的关键单元是类，AOP 中模块化的单元是方面。 方面使关注点（也可以理解为横切关注点）实现模块化，例如跨多种类型和对象的事务管理。

AspectJ 已成长为一个完整且流行的 AOP 框架，Spring 支持在其 AOP 框架中使用以 [AspectJ](https://eclipse.org/aspectj/) 注解编写的 POJO 方面。 由于越来越多的 AOP 框架支持 AspectJ 注解，因此您的 AspectJ 样式的方面更有可能在支持 AspectJ 的其他 AOP 框架中重用。
...
在我们的示例中，我们创建了一个日志记录方面。 要创建方面，您需要在方面类上应用@Aspect 注解并将其注册到`applicationContext.xml`文件中。
...
1.  **咨询之前**：咨询在连接点之前执行，但不具有阻止执行流程前进到连接点的能力（除非它引发异常）。 要使用此建议，请使用上面示例中使用的@Before 注解。
...
5.  **建议周围**：围绕联接点的建议，例如方法调用。 这是最有力的建议。 要使用此建议，请使用@Around 注解。
