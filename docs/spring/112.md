...
AspectJ 已成长为一个完整且流行的 AOP 框架，Spring 支持在其 AOP 框架中使用以 [AspectJ](https://eclipse.org/aspectj/) 批注编写的 POJO 方面。 由于越来越多的 AOP 框架支持 AspectJ 注释，因此您的 AspectJ 样式的方面更有可能在支持 AspectJ 的其他 AOP 框架中重用。
