...
要使用 [spring security oauth2 模块](https://spring.io/projects/spring-security-oauth)创建授权服务器，我们需要使用注释`@EnableAuthorizationServer`并扩展类`AuthorizationServerConfigurerAdapter`。
...
要创建资源服务器组件，请使用`@EnableResourceServer`批注并扩展`ResourceServerConfigurerAdapter`类。
