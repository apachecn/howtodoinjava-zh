...
它可以扫描配置类，文件并将它们加载到 **spring context** 中。 在下面的示例中，执行从`main()`方法开始。 它开始加载所有配置文件，对其进行配置，然后根据`/resources`文件夹中 **application.properties** 文件中的[应用程序属性](https://docs.spring.io/spring-boot/docs/current/reference/html/common-application-properties.html)引导应用程序。
