# Spring AOP AspectJ @Around 注释示例
...
在此**Spring 操作示例**中，我们将学习使用 **Aspectj `@Around`注释**。 `@Around`带注释的方法在与切入点表达式匹配的所有方法之前和之后运行。

在此示例中，我们将创建一个简单的 spring 应用程序，添加记录方面周围的内容，然后基于`@Around`注释中传递的切入点信息调用方面方法。

## 1\. AspectJ @Around 注释用法
...
[@Around 注释](https://eclipse.org/aspectj/doc/next/aspectj5rt-api/org/aspectj/lang/annotation/Around.html)
