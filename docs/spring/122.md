...
在这个 Spring aop 示例中，在使用`<aop:after-returning/>`配置返回建议后，我们将学习使用 **AOP**。 在返回建议后配置为的方法，将在这些方法之后立即运行，这些方法与作为参数传递的切入点表达式匹配，并且通常会终止，即它们不会引发任何异常。

在此示例中，我们将创建简单的 spring 应用程序，添加日志记录方面，然后基于在`<aop:after-returning/>` xml 配置中传递的切入点信息来调用方面方法。

## 返回建议后创建 Spring AOP

要在返回建议后**使用 xml 配置创建**，请按以下方式使用`<aop:after-returning/>`。
