...
在这个 Spring aop 示例中，我们将在使用`<aop:after/>`配置的建议后学习使用 AOP。 配置为事后通知的方法，在与作为参数传递的切入点表达式匹配的方法之后立即运行。

在此示例中，我们将创建简单的 spring 应用程序，添加日志记录方面，然后基于在`<aop:after/>` xml 配置中传递的切入点信息来调用方面方法。
