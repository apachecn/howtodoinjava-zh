...
在本教程中，学习在 Windows 中**在 Windows** 中安装 Docker 容器，**创建 Docker 映像**和**部署 Docker 映像**（作为一个简单的基于 spring boot 的微服务）。 开发人员机器。
...
开发一个简单的 hello world [微服务](//howtodoinjava.com/microservices/microservices-definition-principles-benefits/)进行测试。 我们使用[弹簧靴](//howtodoinjava.com/spring/spring-boot/spring-boot-jersey-example/)和 [Maven](//howtodoinjava.com/maven/) 和 Eclipse 作为 IDE。 添加 [REST](http://restfulapi.net) 端点，以便将该应用程序部署到 Docker 之后，我们可以通过访问其余端点来对其进行测试。
