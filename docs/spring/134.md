...
[**JSR-303**](http://beanvalidation.org/1.0/spec/ "jsr 303") bean 验证是一个规范，其目的是通过注释标准化 Java bean 的验证。 JSR-303 标准的目的是在 Java bean 类中直接使用注释。 这允许直接在要验证的代码中指定验证规则，而不是在单独的类中创建验证规则。 到目前为止，我们了解了使用 [BindingResult.rejectValue（）](//howtodoinjava.com/spring/spring-mvc/spring-mvc-display-validate-and-submit-form-example/ "Spring MVC Display, Validate and Submit Form Example")和[自定义验证器实现](//howtodoinjava.com/spring/spring-mvc/spring-mvc-custom-validator-example/ "Spring MVC Custom Validator Example")在 spring mvc 中进行的验证。 在本示例中，我们将学习**使用基于 JSR-303 标准**的注释来验证 Spring 管理的 bean。

> **阅读更多：[提交表单示例](//howtodoinjava.com/spring/spring-mvc/spring-mvc-display-validate-and-submit-form-example/ "Spring MVC Display, Validate and Submit Form Example") & [自定义验证程序示例](//howtodoinjava.com/spring/spring-mvc/spring-mvc-custom-validator-example/ "Spring MVC Custom Validator Example")**
...
为了使验证真正起作用，您还需要一个实现，例如 [**Hibernate Validator**](http://hibernate.org/validator/ "validator") 。
...
> **阅读更多：[所有受支持的 JSR-303 批注的列表](http://docs.jboss.org/hibernate/stable/validator/reference/en-US/html_single/#section-builtin-constraints "jsr 303 annotations")**
