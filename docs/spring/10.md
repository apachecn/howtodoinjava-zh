...
在本 **spring 5 休眠 5 注释示例**教程中，学习**创建 Spring 5 MVC Web 应用程序**，**处理表单提交**，**集成休眠 5** 连接到后端数据库并为输入表单[字段验证](https://howtodoinjava.com/resteasy/resteasy-bean-validation-using-hibernate-validator-provider/)添加**休眠验证器**。

我们将创建一个简单的屏幕，可以在其中添加用户字段（名称和电子邮件）。 这些详细信息将首先通过休眠进行验证，然后存储在 [HSQL 数据库中。 该页面还将列出所有已存储的用户。](https://howtodoinjava.com/hibernate/hibernate-4-using-in-memory-database-with-hibernate/)
...
随着 [Servlet 3.0 规范](https://download.oracle.com/otndocs/jcp/servlet-3.0-fr-oth-JSpec/)的发布，可以（几乎）没有 xml 配置 Servlet 容器。 为此，Servlet 规范中有[`ServletContainerInitializer`](https://docs.oracle.com/javaee/7/api/javax/servlet/ServletContainerInitializer.html)。 在该类中，您可以注册过滤器，侦听器，servlet 等，就像您在`web.xml`中传统上所做的那样。
...
4.  `@ComponentScan`批注用于指定要扫描的基本软件包。 任何用@Component 和@Configuration 注释的类都将被扫描。
...
*   `LocalSessionFactoryBean`创建一个休眠`SessionFactory`。 这是在 Spring 应用程序上下文中设置共享的 Hibernate SessionFactory 的常用方法。
*   `EnableTransactionManagement`启用 Spring 的注释驱动的交易管理功能。
...
服务和 DAO 层是用`@Service`和`@Repository`注释注释的常规服务组件。 `@Transactional`注释在服务层应用，以支持事务。

> 阅读更多： [@Service 和@Repository 批注](https://howtodoinjava.com/spring/spring-core/how-to-use-spring-component-repository-service-and-controller-annotations/)
...
我希望您能找到这个**春季休眠 Web 应用程序示例**，以开始开发自己的应用程序。 这主要是针对初学者的，但是它将帮助您构建带有注释的任何带有休眠集成示例的 **Spring MVC。**
