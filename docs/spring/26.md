...
了解在 Spring 应用程序中使用基于创建[发布者-订阅者](https://howtodoinjava.com/jms/jms-publish-subscribe-message-example/)的事件。 Spring 对创建[H​​TG2]应用程序事件，发布它们然后在事件处理程序中监听提供了内置支持。 要创建/监听应用程序事件，需要遵循一些简单的准则：
