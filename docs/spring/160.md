...
Spring 安全性基于安全性上下文，本质上是静态的。 从本质上讲，这意味着您无需将其引用注入到 spring 容器中的 bean 或类中。 您只需使用[`SecurityContextHolder.getContext()`](http://static.springsource.org/spring-security/site/docs/3.0.x/apidocs/org/springframework/security/core/context/SecurityContextHolder.html#getContext%28%29)方法即可随时访问 spring 上下文。
